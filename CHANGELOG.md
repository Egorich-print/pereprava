...
`vendor/fernfs` source (a real path dependency), the Tauri icon set (standard
...
Note that `crates/widget/gen/` is *not* kept despite feeding the capability ACL:
it is regenerated by `tauri-build` on every widget build, so it was purged along
with the other artifacts and came back on its own during verification.
